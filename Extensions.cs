...
namespace MapPostprocessor
{
    ...
    public static class NoteExtensions 
    {
        ...
        public static void EncodeToArray<T>(this IWrapper<T> note, Span2D<float> array, int count, float _time, ParamOverride? paramOverride = null) where T : BeatmapGridObject
        {
            ...
            array[count, +index++] = note.Event == null || note.Event.eventType == ReplayDecoder.NoteEventType.good ? 1 : 0;
            array[count, +index++] = note.Event != null && note.Event.eventType == ReplayDecoder.NoteEventType.bad ? 1 : 0;
            array[count, +index++] = note.Event != null && note.Event.eventType == ReplayDecoder.NoteEventType.miss ? 1 : 0;
            array[count, +index++] = note.Event != null && note.Event.eventType == ReplayDecoder.NoteEventType.bomb ? 1 : 0;
            ...
            if (note.Color == 0)
            {
                ...
                encodeInt(array, count, ref index, 0, -2, 13);
            ...
            //encodeInt(array, count, ref index, 0, ()note.ScoringType, 5);
        }
        ...
        public static void EncodeToArray(this WallWrapper note, Span2D<float> array, int count, float _time, ParamOverride? paramOverride = null)
        {
            ...
            array[count, +index++] = note.WallEvent != null ? 1 : 0;
            //array[count, index++] = type == 0 ? param1 * 0.5f : param1;
            //array[count, index++] = type == 0 ? param2 * 0.5f : param2;
            //array[count, index++] = type == 0 ? param3 * 0.5f : param3;
